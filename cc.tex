...
%Collaborative clustering is a recent and trendy machine learning paradigm in which several unsupervised algorithms work together with a goal of mutual improvement to tackle and find the underlying structures in difficult data sets~\cite{Cornuejols201881}. It has numerous applications such as multi-view clustering, the clustering of distributed and heterogeneous data, and multi-expert clustering. While collaborative clustering is similar in spirit with ensemble learning, the lack of supervision makes the collaboration between clustering algorithms more challenging: the success of supervised ensemble methods such as bagging and boosting relies on Condorcet's Jury theorem which states that these methods are sure to achieve good results as long as all classifiers do better than random with a sufficient diversity in their solutions~\cite{Kuncheva03}. However in the case of unsupervised algorithms, the notion of quality is difficult to evaluate and one cannot know if an unsupervised algorithm does better than random.
...
%In the context of horizontal collaborative learning, we consider a finite number of $J$ SOM algorithms $\mathcal{A}^1,...,\mathcal{A}^J$ working on different attributes of a data set made of $d$ numerical attributes
...
%From there, in the collaborative SOM algorithm, when a new input data is presented, the optimization is done to minimize the distance between the data and its best matching unit(s) in each local SOM algorithm $\mathcal{A}^j$ weighted by the neighborhood function. Furthermore, the optimization process also attempts to keep all $J$ self-organizing maps as topologically close as possible. The intended result after collaboration is that, if an observation of the $X^j$-th data set is projected onto the $i^*$-th unit of the $j$-th SOM map, then the same observation of the $X({i})$-th data set should be projected on the same unit  $i^*$ in the $i$-th map or on one of the neighboring unit. In other words, the same prototypes (topologically wise) should capture the same observations in the different maps.
...
%\label{eq:Lj}
%\end{equation}
%and $\beta=(\beta_{i,j})_{i,j}$ is the weigth of the pairwise collaborative $C_{i,j}$ term between the SOM algorithms $i$ and $j$:
%\begin{equation}
%C _{i,j}(w)=	\sum_{l=1}^{N}\sum_{k=1}^{n} \Delta K_{i,j} \| x_l^j - w_{k}^j \|
%\end{equation}
%with
%\begin{equation}
%\Delta K_{i,j}=\left({\mathcal K}{(k, k_c( x_l^j,w^j ) ) } -  {\mathcal K}{(k, k_c( x_l^{i},w^{i} ) ) }  \right)^2
%\label{eq:Cj}
%\end{equation}
...
%$
%w^{j}|_{new} = \operatornamewithlimits{argmin}_{w} [\a_j \cdot  L_j(w) + \sum_{i \neq j} \beta_{i,j} \cdot C _{i,j}(w)  ].
%$
...
%\begin{eqnarray*}
	%\beta_{i,j}|_{new} = \beta_{i,j}|_{old} + 
	%\dfrac
	%{\sum_{l=1}^{N} \sum_{k=1}^{n} {\mathcal K}{(k, k_c( x_l^j,w^j ) )}  }
	%{2  \cdot  \sum_{l=1}^{N} \sum_{k=1}^{n} \Delta K_{i,j} } 
	%\end{eqnarray*}
...
\sectionmark{Optimization under KKT conditions}
\label{sec:opt-w}

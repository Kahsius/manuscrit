...
\begin{document}
    ...
    \section{Clustering collaboratif incrémental}
        ...
        \begin{frame}{SOM incrémentale}
            ...
            \only<2->{\textbf{Problème 1}: Est-il possible de trouver une 
            solution \textbf{conservant la topologie de la carte} ?\\}
            \only<3->{\textbf{Problème 2}: $\lambda$ est \textbf{dépendante du 
            ...
            \onslide<4->{
                ...
                \begin{columns}
                    \begin{column}{0.6\textwidth}
                        \begin{align*}
                            & Q_{local}^m(K_{i,j}) / Q_{collab}^m (\Delta K_{i,j}) \\
                            \Rightarrow\quad & Q_{local}^m(\widetilde{K}_{i,j}) / 
                            Q_{collab}^m (\Delta \widetilde{K}_{i,j})\\
                            \Rightarrow\quad & \widetilde{Q}_{local}^m / 
                            \widetilde{Q}_{collab}^m
                        \end{align*}
                    \end{column}
